...
async def test_main_init(mock_page):
    with (
        patch("database.manager.db_manager.init_db", new_callable=AsyncMock) as mock_init_db,
        patch(
            "database.manager.db_manager.get_setting", new_callable=AsyncMock
        ) as mock_get_setting,
    ):
...
async def test_navigation_dashboard(mock_page):
    with (
        patch("database.manager.db_manager.init_db", new_callable=AsyncMock),
        patch(
            "database.manager.db_manager.get_setting", new_callable=AsyncMock
        ) as mock_get_setting,
    ):
