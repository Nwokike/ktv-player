...
async def main(page: ft.Page):
    ...
    def route_change(e: ft.RouteChangeEvent):
        ...
        if parsed_url.path == "/":
            ...
            page.views.append(
                ft.View("/", [SplashView(on_complete=lambda: page.push_route(dest))], padding=0)
            )
        
        # Onboarding
        elif parsed_url.path == "/onboarding":
            page.views.append(
                ft.View("/onboarding", [OnboardingView(on_complete=lambda: page.push_route("/dashboard"))], padding=0)
            )
        ...
        # Player (Robust parsing)
            ...
            if encoded_url:
                try:
                    ...
                    page.views.append(
                        ft.View(page.route, [PlayerView(url=url, on_back=lambda: page.push_route("/dashboard"))], padding=0)
                    )
